...
var getCurrentAuth = function(){
	redisClient.get(XI_CREDS_KEY, function(err, reply) {
        if (!err) {
            if (reply === null) {
                return false;
            }
            xiCreds = JSON.parse(reply);
            hasXiAuth = true;
            return true;
        }
        log.info("Error initializing xi creds: " + err);
        return false;
    });
};

var setCurrentAuth = function(creds){
	xiCreds = creds;
	hasXiAuth = true;
};
...
var setupHabaneroAuth = function(jwt, xiAccountId, xiAppId, xiAccessToken){
	...
	return when.promise(function(resolve, reject) {
		createHabaneroIdmUser(xiAccountId, xiAppId, xiAccessToken).then(function(idmUser){
            ...
            console.log("Should resolve here");
