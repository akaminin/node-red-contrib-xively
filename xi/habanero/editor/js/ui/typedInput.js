...
(function($) {
    ...
    $.widget( "nodered.typedInput", {
        _create: function() {
            ...
            this.getOptions = this.options.getOptions;
            ...
            if (hasSubOptions) {
                ...
                this.optionSelectSubValue = $('<input/>', 
                    {class:"node-input-rule-input-subvalue", style:"display:inline-block;width:100px;padding-left:10px;", type:"text"}).appendTo(this.uiSelect);
        ...
        value: function(value) {
            if (!arguments.length) {
                ...
                var options = this.typeMap[this.propertyType].options
                if (options && options.length > 0) {
                    if(typeof this.typeMap[this.propertyType].options == "object"){
                        var indexOfObject = arrayObjectIndexOf(options, value, "value"); 
                        if (indexOfObject === -1) {
                            if(options.length > 0){
                                //default to first option
                                this.optionSelectLabel.text(options[0].label);
                            ...
                            this.optionSelectLabel.text(options[indexOfObject].label);
                        }
                    }else{
        ...
        subvalue: function(subvalue) {
            if (!arguments.length) {
                return this.optionSelectSubValue.val();
            } else {
                this.optionSelectSubValue.val(subvalue);
            }
        },
        type: function(type) {
            if (!arguments.length) {
                ...
                if (opt && this.propertyType !== type) {
                    ...
                    if (opt.options) {
                        if (this.optionSelectTrigger) {
                            ...
                            if (opt.options.indexOf(currentVal) !== -1) {
                                ...
                                if (typeof opt.options[0] === 'string') {
                                    this.value(opt.options[0]);
                                } else if(typeof opt.options[0] === "undefined"){
                                    //pass
                                }else{
                                    this.value(opt.options[0].value);
                                }
                            }
                        }
                    } else {
                    ...
                    if(!opt.hasSubvalue){
                        if(this.optionSelectSubValue){
                            this.optionSelectSubValue.hide()
                        }
                    }else{
                        this.optionSelectSubValue.show();
                    }
                    this._resize();
                }
            }
